        ...
        <div className="p-8">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-black text-slate-900 mb-2">
              {isLogin ? 'Karibu Tena' : 'Jisajili Sasa'}
            </h2>
            <p className="text-slate-500 font-medium">
              {isLogin ? 'Ingia ili uendelee na Story Studio' : 'Jiunge na Story Studio kuanza safari yako'}
            </p>
          </div>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-5 mb-6">
            <div>
              <label className="block text-sm font-black text-slate-700 mb-2 uppercase tracking-widest">Barua Pepe au Jina la Mtumiaji</label>
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="Mfano: mwendwa@email.com"
                  required
                />
              </div>
            </div>
            <div>
              <label className="block text-sm font-black text-slate-700 mb-2 uppercase tracking-widest">Nenosiri (Password)</label>
              <div className="relative">
            ...
            {!isLogin && (
              <div className="flex items-start gap-4 pt-2 bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100">
                <input 
                  type="checkbox" 
                  id="terms"
                ...
                />
                <label htmlFor="terms" className="text-xs text-slate-600 leading-relaxed font-bold">
                  Ninakubaliana na <button type="button" onClick={() => setShowTerms(true)} className="text-indigo-600 font-black hover:underline">Masharti ya Huduma (Privacy & Terms)</button>. 
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 px-4 rounded-2xl transition-all shadow-xl shadow-indigo-600/30 flex items-center justify-center gap-3 active:scale-95 text-lg"
            >
              {loading ? <Loader2 className="animate-spin" size={24} /> : null}
              {isLogin ? 'Ingia Sasa' : 'Jiunge Leo'}
            </button>
          </form>
          ...
          <div className="relative flex items-center justify-center mb-6">
            ...
            <div className="relative bg-white px-4 text-xs text-slate-400 font-black tracking-widest">
              AU
            </div>
          </div>
          ...
          <button 
            ...
            disabled={loading}
            className="w-full bg-white border-2 border-slate-100 hover:bg-slate-50 text-slate-700 font-black py-4 px-4 rounded-2xl transition-all flex items-center justify-center gap-4 shadow-sm active:scale-95"
          >
